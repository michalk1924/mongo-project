...
function Page() {
    ...
    const fetchProducts = async () => {
        setLoading(true);
        try {
            const data = await booksService.getAllProducts();
            setProducts(data);
        } catch (error: any) {
    ...
    const handleAddBook = async (newBook: Book) => {
        try {
            const addedBook = await ProductsService.createBook(newBook);
            setProducts((prevProducts) => [...prevProducts, addedBook]);
    ...
    const handleDeleteBook = async (id: number) => {
        try {
            await ProductsService.deleteBook(id);
            setProducts((Products) => Products.filter((book) => book.id !== id));
        } catch (error: any) {
